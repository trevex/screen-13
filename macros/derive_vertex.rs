...
pub fn derive_vertex(ast: syn::DeriveInput) -> Result<TokenStream> {
    ...
    let crate_ident = match crate_name("screen-13") {
        Ok(FoundCrate::Itself) => Ident::new("screen_13", Span::call_site()),
    ...
    Ok(TokenStream::from(quote! {
        #[allow(unsafe_code)]
        impl #crate_ident::driver::vertex::Vertex for #struct_name {
            #[inline]
            fn layout(input_rate: #crate_ident::driver::ash::vk::VertexInputRate) -> #crate_ident::driver::vertex::DerivedVertexLayout {
                use std::collections::HashMap;
                use #crate_ident::driver::ash::vk;
                use #crate_ident::driver::vertex::DerivedVertexAttribute;

                #attributes

                #crate_ident::driver::vertex::DerivedVertexLayout {
                    attributes,
                    input_rate,
